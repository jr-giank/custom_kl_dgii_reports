...
class DgiiReport(models.Model):
    ...
    def _get_invoices(self, states, types):
        ...
        # Append pending invoces (fiscal_status = Partial, payment_state = Paid)
    ...
    def formated_rnc_cedula(self, vat):
        ...
        clean_vat = vat.strip().replace('-', '')
        if not len(clean_vat) in [9, 11]:
            return False

        id_type = 1 if len(clean_vat) == 9 else 2
        return (clean_vat, id_type) if not clean_vat.isspace() else False
    ...
    def _generate_txt(self, records, qty, report_number):

        company_vat = (
            str(self.company_id.vat).ljust(9)
            if len(self.company_id.vat) == 9
            else str(self.company_id.vat).ljust(11)
        )
        period = dt.strptime(self.name.replace('/', ''), '%m%Y').strftime('%Y%m')

        header = "{}|{}|{}|{}".format(report_number, company_vat, period, qty) + '\n'
        data = header + records

        file_path = '/tmp/DGII_{}_{}_{}.txt'.format(report_number, company_vat, period)
        ...
        self.write(
            {
                '{}_filename'.format(report_type[report_number]): file_path.replace('/tmp/', ''),
                '{}_binary'.format(report_type[report_number]): base64.b64encode(open(file_path, 'rb').read()),
            }
        )
    ...
    def _include_in_current_report(self, invoice):
        ...
        period = dt.strptime(self.name, '%m/%Y')
        same_minor_period = (payment_date.month, payment_date.year) <= (
            period.month,
            period.year,
        )
    ...
    def _compute_606_data(self):
        for report in self:
            current_purchase_lines = self.env['dgii.reports.purchase.line'].search(
                [('dgii_report_id', '=', report.id)]
            )
            current_purchase_lines.unlink()
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    'payment_date': inv.payment_date if invoice_should_be_included else False,
                    ...
                    'isr_withholding_type': inv.isr_withholding_type if (invoice_should_be_included and (inv.withholded_itbis or inv.income_withholding)) else False,
                    'withholded_itbis': inv.withholded_itbis if invoice_should_be_included else 0,
                    'income_withholding': inv.income_withholding if invoice_should_be_included else 0,
                ...
                current_purchase_lines.create(values)
                report_data += self.process_606_report_data(values) + '\n'
            self._generate_txt(report_data, line_number, "606")
    ...
    def include_payment(invoice_id, payment_id):
        ...
        if payment_date and invoice_date:
            return (
                True
                if (payment_date.year <= invoice_date.year) and (payment_date.month <= invoice_date.month)
                else False
            )
        else:
            return False
    ...
    def _is_customer_invoice_refund(self, inv):
        return inv.move_type == 'out_refund'

    def _get_sale_payments_forms(self, invoice_id):
        payments_dict = self._get_payments_dict()

        if self._is_customer_invoice(invoice_id):
    ...
    def _process_operation_dict(self, operation_dict, invoice):
        if invoice.l10n_latam_document_type_id and invoice.move_type != 'out_refund':
            operation_dict[invoice.l10n_latam_document_type_id.doc_code_prefix]['qty'] += 1
            operation_dict[invoice.l10n_latam_document_type_id.doc_code_prefix]['amount'] += invoice.amount_untaxed_signed
    ...
    def _process_income_dict(self, income_dict, invoice):
    ...
    def _compute_607_data(self):
        for report in self:
            current_sale_lines = self.env['dgii.reports.sale.line'].search([('dgii_report_id', '=', report.id)])
            current_sale_lines.unlink()
            ...
            line_number = 0
            excluded_line = line_number
            payment_dict = self._get_payments_dict()
            csmr_dict = self._get_csmr_vals_dict()

            report_data = ''
            for inv in invoice_ids:
                operations_dict = self._process_operation_dict(self._get_607_operations_dict(), inv)
                income_dict = self._process_income_dict(self._get_income_type_dict(), inv)
                ...
                values = {
                    ...
                    if inv.l10n_do_origin_ncf and inv.l10n_do_origin_ncf[-10:-8] in ['01', '02', '14', '15'] else False,
                    'income_type': inv.l10n_do_income_type,
                    'invoice_date': inv.invoice_date,
                    'withholding_date': inv.withholding_date if invoice_should_be_included else False,
                    'invoiced_amount': inv.amount_untaxed_signed,
                    'invoiced_itbis': inv.invoiced_itbis,
                    'third_withheld_itbis': inv.third_withheld_itbis
                    if invoice_should_be_included else 0,
                    'perceived_itbis': 0,
                    ...
                    'invoice_partner_id': inv.partner_id.id,
                    'invoice_id': inv.id,
                    'credit_note': True if self._is_customer_invoice_refund(inv) else False,
                    'cash': payments.get('cash') * -1 if self._is_customer_invoice_refund(inv) else payments.get('cash'),
                    'bank': payments.get('bank') * -1 if self._is_customer_invoice_refund(inv) else payments.get('bank'),
                    'card': payments.get('card') * -1 if self._is_customer_invoice_refund(inv) else payments.get('card'),
                    'credit': payments.get('credit') * -1 if self._is_customer_invoice_refund(inv) else payments.get('credit'),
                    'swap': payments.get('swap') * -1 if self._is_customer_invoice_refund(inv) else payments.get('swap'),
                    'bond': payments.get('bond') * -1 if self._is_customer_invoice_refund(inv) else payments.get('bond'),
                    'others': payments.get('others') * -1 if self._is_customer_invoice_refund(inv) else payments.get('others')
                }
                ...
                line_number += 1
                values.update({'line': line_number})
                current_sale_lines.create(values)
                if (
                ...
                for k in payment_dict:
                    payment_dict[k] += (
                        payments[k] * -1
                        if self._is_customer_invoice_refund(inv)
                        else payments[k]
                    )

                for k in operations_dict:
                    self.env['dgii.reports.sale.summary'].create(operations_dict[k])

                self._set_csmr_fields_vals(csmr_dict)
                self._set_payment_form_fields(payment_dict)
                self._set_income_type_fields(income_dict)
                self._generate_txt(report_data, line_number - excluded_line, "607")
    ...
    def _compute_608_data(self):
        for report in self:
            current_cancel_lines = self.env['dgii.reports.cancel.line'].search([('dgii_report_id', '=', report.id)])
            current_cancel_lines.unlink()
            ...
            for inv in invoice_ids:
                ...
                current_cancel_lines.create(values)
                report_data += self.process_608_report_data(values) + '\n'

            self._generate_txt(report_data, line_number, "608")
    ...
    def _compute_609_data(self):
        for report in self:
            current_exterior_lines = self.env['dgii.reports.exterior.line'].search([('dgii_report_id', '=', report.id)])
            current_exterior_lines.unlink()

            invoice_ids = self._get_invoices(
                ['posted'],
                ['in_invoice', 'in_refund']
            ).filtered(lambda inv: (inv.is_exterior is True))
            ...
            for inv in invoice_ids:
                ...
                values = {
                    ...
                    'invoiced_amount': inv.amount_untaxed_signed,
                    'isr_withholding_date': inv.payment_date if inv.income_withholding else False,
                    'presumed_income': 0,  # Pendiente
                    'withholded_isr': inv.income_withholding if inv.payment_date else 0,
                    'invoice_id': inv.id,
                }
                current_exterior_lines.create(values)
                report_data += self.process_609_report_data(values) + '\n'

            self._generate_txt(report_data, line_number, "609")

    def _generate_report(self):
        current_report = self.env['dgii.reports.sale.summary'].search(
            [('dgii_report_id', '=', self.id)]
        )
    ...
    def generate_report(self):
        if self.state == 'generated':
            action = self.env.ref(
                'custom_kl_dgii_reports.dgii_report_regenerate_wizard_action'
            ).sudo().read()[0]
    ...
    def _invoice_status_sent(self):
        for report in self:
            ...
            for inv in invoice_ids:
                if inv.state in ['cancel'] or inv.payment_state in ['paid'] and self._include_in_current_report(inv):
    ...
    def confirmation_to_sent(self):
        confirmation = self.env.ref('custom_kl_dgii_reports.dgii_report_confirmation_wizard_action').sudo().read()[0]
        confirmation['context'] = {'default_report_id': self.id}
        return confirmation

    def _reverse_invoice_status_sent(self):
        for report in self:
            PurchaseLine = self.env['dgii.reports.purchase.line']
            SaleLine = self.env['dgii.reports.sale.line']
            CancelLine = self.env['dgii.reports.cancel.line']
    ...
    def _reverse_update_pending_invoices(self):
        invoice_ids = self.env["account.move"].search([
            ("payment_state", "=", "paid"),
            ("fiscal_status", "=", "normal"),
            ("payment_date", "=", False),
        ])
        invoice_ids.write({"fiscal_status": "normal"})

    def reset_report(self):
        self.state = 'draft'
        self._reverse_invoice_status_sent()
        self._reverse_update_pending_invoices()
    ...
    def get_606_tree_view(self):
        return {
            ...
            'view_id':
                self.env.ref('custom_kl_dgii_reports.dgii_report_purchase_line_tree').id,
    ...
    def get_607_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('custom_kl_dgii_reports.dgii_report_sale_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)],
        }
    ...
    def get_608_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('custom_kl_dgii_reports.dgii_cancel_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)],
        }
    ...
    def get_609_tree_view(self):
        return {
            ...
            'type': 'ir.actions.act_window',
            'view_id':
                self.env.ref('custom_kl_dgii_reports.dgii_exterior_report_line_tree').id,
            'domain': [('dgii_report_id', '=', self.id)],
        }
